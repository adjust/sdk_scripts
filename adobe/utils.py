...
def check_submodule_dir(platform, submodule_dir):
    if not os.path.isdir(submodule_dir) or not os.listdir(submodule_dir):
        error('[{0}] submodule folder empty. Did you forget to run \'git submodule update --init --recursive\'?'.format(
            platform))
        exit()
...
def adobe_air_compc_test(root_dir, build_dir):
    ...
    execute_command([compc, '-source-path', default_src_dir, '-external-library-path',
                     external_lib_path, '-include-classes', 'com.adjust.test.AdjustTest', '-directory=true', '-output',
                     output_dir])
...
def adobe_air_adt_sdk(version):
    ...
    execute_command(
        [adt, '-package', '-target', 'ane', '../Adjust-{0}.ane'.format(version), 'extension.xml', '-swc', 'Adjust.swc',
...
def adobe_air_adt_test(root_dir, build_dir, version):
    ...
    execute_command(
        [adt, '-package', '-target', 'ane', '{0}/AdjustTest-{1}.ane'.format(root_dir, version), 'extension.xml', '-swc',
         'AdjustTest.swc',
...
def adobe_air_amxmlc_example(version):
    execute_command(
        ['amxmlc', '-external-library-path+=lib/Adjust-{0}.ane'.format(version), '-output=Main.swf', '--', 'Main.as'])
...
def adobe_air_package_ipa_file(prov_profile_path, keystore_file_path, example_app_xml_file):
    ...
    command = """adt -package -target ipa-debug -provisioning-profile {0} -storetype pkcs12 -keystore {1} Main.ipa {2} Main.swf -extdir lib""".format(
